...
$(window).on("load", function() {
	...
	$("#submit-search").on("click", function() {
		$(".chart-bar-subdimensi-iae").html("");
		$(".chart-bar-subdimensi-iae").append(
			"<canvas id='subdimensi-iae' width='850' height='500'></canvas>"
		);
		clickedFilter();
	});
	ajaxData(start, end);
	...
	$.ajax({
		...
		success: function(data) {
			...
			$("#subdimensi-value").append(
				"<td colspan='2'>" + data.subDimensi[1].nama_sub_dimensi + "</td>"
			);

			$.each(data["subDimensi"][1]["nilai"], function(index, nilaisubdimensi) {
				$("#subdimensi-value").append(
					"<td style='border: 1px solid black'>" + nilaisubdimensi + "</td>"
				);
			});

			$.each(data["indikator_sd"][1], function(index, indikator) {
				$("#subdimensi-table").append(
					"<tr id='indikator-" +
						index +
			...
			window.myLineChart = new Chart($("#subdimensi-iae"), {
				type: "bar",
				data: {
					labels: data["range_tahun"],
					datasets: [
						{
							label: data["subDimensi"][1]["nama_sub_dimensi"],
							type: "line",
							borderColor: "#654321",
							data: data["subDimensi"][1]["nilai"],
							fill: false
						}
					]
				},
			...
			for (var i = 0; i < data["indikator_sd"][1].length; i++) {
				myLineChart.data.datasets.push({
					label: data["indikator_sd"][1][i]["nama_indikator"],
					type: "bar",
					backgroundColor: rgb[i],
					data: data["indikator_sd"][1][i]["nilai"]
				});
			}
